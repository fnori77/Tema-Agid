...
class Breadcrumb_Trail {
	...
	public function trail() {
		if(str_contains($_SERVER['REQUEST_URI'],'luoghi'))
		{
			$dato = array_search('Luoghi',$this->items);
			
			if(!$dato){

				array_splice($this->items, 2, 0, array( "<a href='".home_url("/vivere-il-comune/luoghi")."'>".__("Luoghi", "design_comuni_italia")."</a>"));
			}
			
		}
		if(str_contains($_SERVER['REQUEST_URI'],'eventi'))
		{
			...
			{
				array_splice($this->items, 2, 0, array( "<a href='".home_url("/vivere-il-comune/eventi")."'>".__("Eventi", "design_comuni_italia")."</a>"));
			}
		}
	...
	protected function add_items() {
		...
		// If not viewing the front page.
            ...
            if ( is_singular() ) {
				...
				if (get_post_type() == 'servizio') {
					...
					if($terms){
					  ...
					  foreach ($terms as $term) {
						  $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url("/servizi-categoria/".$term->slug), $term->name );
					  }
					}
				...
				if (get_post_type() == 'page' && str_contains($_SERVER['REQUEST_URI'], 'eventi')) {
    $url_to_split = explode('l=', $_SERVER['REQUEST_URI']);

    if ($url_to_split[1]) {
        $info_tipo = (isset($_GET['l']) ? $_GET['l'] : '');
        $this->items[] = "<a href='" . home_url("vivere-il-comune") . "'>" . __("Vivere il Comune", "design_comuni_italia") . "</a>";

        $penultimate_active = false; // Inizializza la variabile che indica se la penultima voce è attiva

        foreach ($terms as $term) {
            $term_name = str_replace('-', ' ', $term->name); // Sostituisci trattini con spazi vuoti
            $term_name = ucfirst($term_name); // Formatta la prima lettera in maiuscolo

            $term_link = "/eventi?e=$term->slug";

            // Verifica se il termine corrente è attivo
            $is_term_active = ($term->slug == $url_to_split[1]);

            // Aggiungi la voce al breadcrumb, rendendola non cliccabile se attiva
            $this->items[] = $is_term_active ? esc_html($term_name) : sprintf('<a href="%s">%s</a>', esc_url($term_link), $term_name);

            // Aggiorna lo stato della penultima voce
            if ($is_term_active) {
                $penultimate_active = true;
            }
        }

        // L'ultima voce del breadcrumb senza link
		
        $last_term_name = str_replace('-', ' ', $url_to_split[1]); // Sostituisci trattini con spazi vuoti
        $last_term_name = ucfirst($last_term_name); // Formatta la prima lettera in maiuscolo
        $this->items[] = esc_html($last_term_name);
		

        // Se la penultima voce è attiva, rendila non cliccabile
		
		
        if ($penultimate_active) {
			
            array_splice($this->items, -2, 1, esc_html($penultimate_term_name));
        }
    ...
    return;
				...
				if (get_post_type() == 'evento') {
						...
						
					{
						...
						{ 
							...
							$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( "/eventi?l=$term->slug" ), ucfirst($term->name)); 
						} 
				}
					$this->items[] = get_the_title();
					return;
				...
				if(get_post_type() == 'luogo')
				{
					...
					{ 
						...
						foreach ($terms as $term) 
						{ 
							$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( "/luoghi?l=$term->slug" ), ucfirst($term->name)); 
						} 
					}
				...
				if (get_post_type() == 'notizia') 
				{
					...
					if($terms)
					{ 
						$info_tipo = (isset($_GET['tipo']) ? $_GET['tipo'] : '');
						$custom = ($info_tipo == '' ? '/novita' : "/novita?tipo=$info_tipo");
						$this->items[] =  sprintf( '<a href="%s">%s</a>', esc_url( $custom ), 'Novità');
						foreach ($terms as $term) 
						{ 
							$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( "/novita?tipo=$term->slug" ), ucfirst($term->name)); 
						} 
					}
					$this->items[] = get_the_title(); return; 
				}
			    $group_name = dci_get_group_name(get_post_type());
			    //console_log($group_name);
			    switch ($group_name) {
                    case 'Vivere il comune' :
                        $this->items[] =  "<a href='".home_url("vivere-il-comune")."'>".__("Vivere aail Comune", "design_comuni_italia")."</a>";
                        $this->items[] = get_the_title();
                        return;
                        break;
                    case 'Amministrazione':
						$tipi_organizzazione = get_the_terms(get_the_ID(), 'tipi_unita_organizzativa');
						
						$tipo_organizzazione = (is_array($tipi_organizzazione) ? array_column($tipi_organizzazione, 'name') ?? null : $tipi_organizzazione);
						$nome = (is_object($tipi_organizzazione[0]) && $tipi_organizzazione[0]->name  == 'ufficio' ? 'uffici' : $tipi_organizzazione[0]);
						
						$prefix = "_dci_unita_organizzativa_";
						$parent_post_organizzativo = dci_get_meta("unita_organizzativa_genitore", $prefix, $post->ID);
						if(!empty($parent_post_organizzativo)){
							$tipi_organizzazione_test = get_post($parent_post_organizzativo[0]);
							
							$nome_parent_organizzazione = (is_object($tipi_organizzazione_test) && isset($tipi_organizzazione_test->post_title)  ? $tipi_organizzazione_test->post_title : $tipi_organizzazione_test);
						
						}
												 if (get_post_type() == 'persona_pubblica') {
							 $tipo_incarico = dci_get_meta("tipo_incarico");
        // Modifica in base al tipo di incarico
        if ($tipo_incarico == 'Amministrativo') {
            $nome_slug = 'personale-amministrativo';
            $nome_name = 'Personale Amministrativo';
        } else if ($tipo_incarico == 'Politico'){
            $nome_slug = 'politici';
            $nome_name = 'Politici';
        }
    } else 
						{
						$nome = (is_object($nome) ? ($nome->slug == 'area' ? 'aree-amministrative'  : $nome->slug ) : $nome);
						$nome_slug = (is_object($nome) ? $nome->slug : $nome);
 						$nome_name = (is_object($nome) ? $nome->name : ucfirst($nome));
						$nome_name = str_replace('-', ' ', $nome_name);
						}
						$this->items[] =  "<a href='".home_url("amministrazione")."'>".__("Amministrazione", "design_comuni_italia")."</a>";
		
						if($nome_slug == 'persona_pubblica')
						{
							$this->items[] =  "<a href='".home_url($nome_slug)."'>".__("$nome_name", "design_comuni_italia")."</a>";
						}
						else
						{
							$this->items[] =  "<a href='".home_url("amministrazione")."/$nome_slug'>".__($nome_name, "design_comuni_italia")."</a>";
						}
						//test
						if(!empty($nome_parent_organizzazione)){
						
							$nome_parent_organizzazione = (is_object($nome_parent_organizzazione) ? ($nome_parent_organizzazione->post_title == 'area' ? 'aree-amministrative'  : $nome_parent_organizzazione->post_title ) : $nome_parent_organizzazione);
							$nome_parent_organizzazione_slug = (is_object($nome_parent_organizzazione) ? $nome_parent_organizzazione->post_title : $nome_parent_organizzazione);
							 $nome_parent_organizzazione_name = (is_object($nome_parent_organizzazione) ? $nome_parent_organizzazione->post_title : ucfirst($nome_parent_organizzazione));
							$nome_parent_organizzazione_name = str_replace('-', ' ', $nome_parent_organizzazione_name);
						if($nome_parent_organizzazione_slug == 'persona_pubblica')
							{
								$this->items[] =  "<a href='".home_url($nome_parent_organizzazione_slug)."'>".__("$nome_parent_organizzazione_name", "design_comuni_italia")."</a>";
							}
							else
							{
								$this->items[] =  "<a href='".home_url("amministrazione")."/unita_organizzativa/$nome_parent_organizzazione_slug'>".__($nome_parent_organizzazione_name, "design_comuni_italia")."</a>";
							}	
						}
				...
				$info_tipo = (isset($_GET['tipo']) ? $_GET['tipo'] : 'notizia');
				if(isset($_GET['tipo']))
				{
					$this->items[] =  "<a href='".home_url("novita")."'>".__("Novità", "design_comuni_italia")."</a>";
                    $this->items[] = ucfirst($info_tipo);
				}
				else
				{
					if (isset($_GET['l'])) {
    $url = home_url("vivere-il-comune");
    $url = str_replace('-', ' ', $url);

    $this->items[] = "<a href='" . $url . "'>" . __("Vivere il Comune", "design_comuni_italia") . "</a>";

    // Sostituisci i trattini con spazi vuoti in ucfirst($_GET['l'])
    $term_name = ucfirst(str_replace('-', ' ', $_GET['l']));
    
    $this->items[] = $term_name;
					}
					else
					{
				...
				
			{
                ...
                elseif ( is_category() || is_tag() || is_tax() ){
                    ...
                    if (is_tax(array("categorie_servizio"))){
                        ...
                        $this->items[] = "<a href='".home_url("documenti-e-dati")."'>".__("Amministrazione", "design_comuni_italia")."</a>";
$this->items[] = "<a href='".home_url("documenti-e-dati")."'>".__("Documenti e dati", "design_comuni_italia")."</a>";
